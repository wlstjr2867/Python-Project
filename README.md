...
(녹화시 유효성 검사 창은 안나오기 때문에 게시글 작성과 게시글 상세 조회 설명에 코드 줄 번호 기재함)
...
각 페이지는 QWidget 단위로 분리하여 구성하였으며,
페이지 간 이동은 Signal-Slot 방식으로 처리하여
페이지 간 의존성을 최소화했습니다.


데이터베이스 로직은 DBManager 클래스로 분리하여
화면 로직과 데이터 로직을 분리했습니다.
